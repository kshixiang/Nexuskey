    ...
    try {
      ...
      if (!displayVersion) {
        await settingsApi.openExternal(
          "https://github.com/SQAI/Nexuskey/releases",
        );
        return;
      }

      await settingsApi.openExternal(
        `https://github.com/SQAI/Nexuskey/releases/tag/${displayVersion}`,
      );
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <img src={appIcon} alt="NexusKey" className="h-5 w-5" />
          ...
          {/* managed 构建：发行方统一推送安装包，应用内不暴露上游更新检查按钮，
              避免误导用户去访问 endpoint 不可达的 OTA 更新清单。 */}
        ...
        {!isManagedModeEnabled() && hasUpdate && updateInfo && (
          <motion.div
