...
///
/// Input (from NexusKey UI / database):
/// ```json
...
/// `providers:` dict. NexusKey treats those as read-only — writes have to
...
/// - `custom_providers:` list entries (writable by NexusKey)
...
/// keeping NexusKey free to edit it. Models are denormalized from the YAML
...
/// list — i.e. it is a read-only overlay NexusKey must not touch.
...
pub fn set_provider(
        ...
        .find(|p| p.get("name").and_then(|n| n.as_str()) == Some(name))
    {
        ...
        // this merge, a NexusKey edit to an unrelated field would silently
...
// NexusKey fills that gap by reading/writing the whole file as a markdown
...
mod tests {
    ...
    ///
    /// Saves and restores `NEXUSKEY_TEST_HOME` to avoid interfering with
    /// parallel tests in other modules.
    fn with_test_home<T>(test_fn: impl FnOnce() -> T) -> T {
        let _guard = test_guard();
        let tmp = tempfile::tempdir().unwrap();
        let old_test_home = std::env::var_os("NEXUSKEY_TEST_HOME");
        std::env::set_var("NEXUSKEY_TEST_HOME", tmp.path());
        let result = test_fn();
        match old_test_home {
            Some(value) => std::env::set_var("NEXUSKEY_TEST_HOME", value),
            None => std::env::remove_var("NEXUSKEY_TEST_HOME"),
        }
        result
    }
...
";
    ...
    fn set_provider_preserves_unknown_fields_on_update() {
        ...
        // Hermes Web UI; a later NexusKey edit must not strip them — set_provider
...
";
    ...
    fn set_memory_enabled_preserves_other_fields() {
        ...
        // a NexusKey toggle would silently wipe those fields.
        with_test_home(|| {
            let yaml = "\
memory:
