...
/// 将 `/path/to/NexusKey.app/Contents/MacOS/NexusKey` 转换为 `/path/to/NexusKey.app`
#[cfg(target_os = "macos")]
...
fn get_auto_launch() -> Result<AutoLaunch, AppError> {
    let app_name = crate::app_identity::APP_PRODUCT_NAME;
    let exe_path =
...
mod tests {
    ...
    fn test_get_macos_app_bundle_path_valid() {
        let exe_path = std::path::Path::new("/Applications/NexusKey.app/Contents/MacOS/NexusKey");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from("/Applications/NexusKey.app"))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_with_spaces() {
        let exe_path =
            std::path::Path::new("/Users/test/My Apps/NexusKey.app/Contents/MacOS/NexusKey");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from(
                "/Users/test/My Apps/NexusKey.app"
            ))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_not_in_bundle() {
        let exe_path = std::path::Path::new("/usr/local/bin/nexuskey");
        let result = get_macos_app_bundle_path(exe_path);
    ...
    fn test_get_macos_app_bundle_path_dev_build() {
        ...
        let exe_path = std::path::Path::new("/Users/dev/project/target/debug/nexuskey");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(result, None);
    }
}
