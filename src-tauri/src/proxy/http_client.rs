...
/// 本应用内置代理当前监听的端口
static NEXUSKEY_PROXY_PORT: OnceCell<RwLock<u16>> = OnceCell::new();

/// 设置本应用内置代理的监听端口
///
/// 应在代理服务器启动时调用，以便系统代理检测能正确识别自己的端口
pub fn set_proxy_port(port: u16) {
    if let Some(lock) = NEXUSKEY_PROXY_PORT.get() {
        if let Ok(mut current_port) = lock.write() {
            *current_port = port;
            log::debug!("[GlobalProxy] Updated app proxy port to {port}");
        }
    } else {
        let _ = NEXUSKEY_PROXY_PORT.set(RwLock::new(port));
        log::debug!("[GlobalProxy] Initialized app proxy port to {port}");
    }
}
...
fn get_proxy_port() -> u16 {
    NEXUSKEY_PROXY_PORT
        ...
        .map(|port| *port)
        .unwrap_or(crate::app_identity::APP_DEFAULT_PROXY_PORT) // 默认端口作为回退
}
...
fn proxy_points_to_loopback(value: &str) -> bool {
    ...
    // 检查是否指向 NexusKey 自己的代理端口
    // 只有指向自己的代理才需要跳过，避免递归
    fn is_app_proxy_port(port: Option<u16>) -> bool {
        let app_port = get_proxy_port();
        port == Some(app_port)
    }

    if let Ok(parsed) = url::Url::parse(value) {
        if let Some(host) = parsed.host_str() {
            // 只有当主机是 loopback 且端口是 NexusKey 的端口时才返回 true
...
mod tests {
    ...
    fn test_proxy_points_to_loopback() {
        // 设置 NexusKey 代理端口为 15721（默认值）
        set_proxy_port(15721);

        // 只有指向 NexusKey 自己端口的 loopback 地址才返回 true
