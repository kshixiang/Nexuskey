...
/// 未管理的 Skill（在应用目录中发现但未被 NexusKey 管理）
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub struct McpRoot {
    ...
    /// Cursor IDE — MCP 仅用于 Nexuskey 内标注；同步至 Cursor 需手动配置
    #[serde(default, skip_serializing_if = "McpConfig::is_empty")]
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        match normalized.as_str() {
            ...
            other => Err(AppError::localized(
                "unsupported_app",
                format!("不支持的应用标识: '{other}'。可选值: claude, codex, gemini, opencode, openclaw, hermes, cursor。"),
                format!("Unsupported app id: '{other}'. Allowed: claude, codex, gemini, opencode, openclaw, hermes, cursor."),
            )),
        }
    }
}
...
impl MultiAppConfig {
    ...
    pub fn load() -> Result<Self, AppError> {
        ...
        if !config.apps.contains_key("cursor") {
            config
                .apps
                .insert("cursor".to_string(), ProviderManager::default());
            updated = true;
        }
    ...
    pub fn save(&self) -> Result<(), AppError> {
        let config_path = get_app_config_path();
        // 先备份旧版（若存在）到 ~/.nexuskey/config.json.bak，再写入新内容
        if config_path.exists() {
    ...
    pub fn migrate_mcp_to_unified(&mut self) -> Result<bool, AppError> {
        ...
        for app in [
            ...
            let old_servers = match app {
                ...
                AppType::Cursor => continue,   // Cursor unified MCP came later; skip legacy slice
            };
...
mod tests {
    ...
    impl TempHome {
        fn new() -> Self {
            ...
            let original_test_home = env::var("NEXUSKEY_TEST_HOME").ok();

            env::set_var("HOME", dir.path());
            env::set_var("USERPROFILE", dir.path());
            env::set_var("NEXUSKEY_TEST_HOME", dir.path());
    ...
    impl Drop for TempHome {
        fn drop(&mut self) {
            ...
            match &self.original_test_home {
                Some(value) => env::set_var("NEXUSKEY_TEST_HOME", value),
                None => env::remove_var("NEXUSKEY_TEST_HOME"),
            }
        }
    }
