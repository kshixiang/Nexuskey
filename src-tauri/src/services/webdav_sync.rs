...
const PROTOCOL_FORMAT: &str = "nexuskey-webdav-sync";
...
fn detect_system_device_name() -> Option<String> {
    let env_name = ["NEXUSKEY_DEVICE_NAME", "COMPUTERNAME", "HOSTNAME"]
        .iter()
...
mod tests {
    ...
    fn remote_dir_segments_uses_current_layout() {
        ...
        assert_eq!(segs, vec!["nexuskey-sync", "v2", "db-v6", "default"]);
    }
    ...
    fn remote_dir_segments_uses_legacy_layout() {
        ...
        assert_eq!(segs, vec!["nexuskey-sync", "v2", "default"]);
    }
