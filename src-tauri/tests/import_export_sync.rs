...
fn sync_codex_provider_writes_auth_and_config() {
    ...
    let auth_path = nexuskey::get_codex_auth_path();
    let config_path = nexuskey::get_codex_config_path();
...
"#;
    nexuskey::write_codex_live_atomic(&legacy_auth, Some(legacy_config))
...
"#
    ...
    let toml_text =
        fs::read_to_string(nexuskey::get_codex_config_path()).expect("read config.toml");
...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    let path = nexuskey::get_codex_config_path();
    ...
    nexuskey::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = nexuskey::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        nexuskey::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        nexuskey::AppError::McpValidation(msg) => {
            assert!(
...
fn sync_codex_provider_missing_auth_returns_error() {
    ...
    // 确认未产生任何 live 配置文件
    assert!(
        !nexuskey::get_codex_auth_path().exists(),
        "auth.json should not be created on failure"
    );
    assert!(
        !nexuskey::get_codex_config_path().exists(),
...
"#;
    ...
    nexuskey::write_codex_live_atomic(&auth, Some(config_text))
    ...
    let stored_auth: serde_json::Value =
        nexuskey::read_json_file(&auth_path).expect("read auth");
...
"#;
    ...
    let err = nexuskey::write_codex_live_atomic(&auth, Some(config_text))
        .expect_err("config write should fail when target is directory");
    match err {
        nexuskey::AppError::Io { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "io error path should point to config.toml"
            );
        }
        nexuskey::AppError::IoContext { context, .. } => {
            assert!(
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    let changed = nexuskey::import_from_codex(&mut config).expect("import codex");
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    nexuskey::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = nexuskey::get_claude_mcp_path();
...
fn import_from_claude_merges_into_config() {
    ...
    let changed = nexuskey::import_from_claude(&mut config).expect("import from claude");
...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = home.join(".nexuskey").join("config.json");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(".nexuskey");
...
fn sync_gemini_packycode_sets_security_selected_type() {
    ...
    // security field is written to ~/.gemini/settings.json, not ~/.nexuskey/settings.json
...
fn export_sql_returns_error_for_invalid_path() {
    ...
    let invalid_parent = if cfg!(windows) {
        std::env::temp_dir().join("nexuskey-test-invalid<>dir")
    } else {
...
#[test]
fn import_sql_rejects_non_nexuskey_backup() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    let import_path = home.join("not-nexuskey-export.sql");
    fs::write(&import_path, "CREATE TABLE x (id INTEGER);").expect("write import sql");

    let err = state
        .db
        .import_sql(&import_path)
        .expect_err("non-nexuskey sql should be rejected");
...
#[test]
fn import_sql_accepts_nexuskey_exported_backup() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    let state = create_test_state_with_config(&config).expect("create test state");
    let export_path = home.join("nexuskey-export.sql");
    state
        .db
        .export_sql(&export_path)
        .expect("export should succeed");
